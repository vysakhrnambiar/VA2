...
# Resend Email Configuration & Other App Config
APP_CONFIG = {
    ...
    "FASTAPI_DISPLAY_API_URL": os.getenv("FASTAPI_DISPLAY_API_URL") # New variable for display API
}
...
state_just_changed_to_sending = False  # Flag to track state change for response.create
if wake_word_active: 
...
def set_app_state_main(new_state): 
    ...
    with state_lock:
        if current_app_state != new_state:
            ...
            if new_state == STATE_SENDING_TO_OPENAI: # Set flag when changing to SENDING_TO_OPENAI
...
def continuous_audio_pipeline(openai_client_ref): 
    global state_just_changed_to_sending # Make sure global is recognized in this scope
        ...
        while True: 
            ...
            try:
                ...
                if len(audio_bytes_24k) != expected_len:
                    log(f"WARN: Mic read {len(audio_bytes_24k)} bytes, expected {expected_len}. Skipping chunk.")
                    continue
            except IOError as e:
            ...
            if current_processing_state == STATE_LISTENING_FOR_WAKEWORD and wake_word_active:
                if SCIPY_AVAILABLE and wake_word_detector_instance:
                    ...
                    if num_samples_16k > 0:
                        ...
                        # chunk_counter += 1 # Debug logging can be re-enabled if needed
                        ...
                        # log("WARN: Resampling for WW resulted in 0 samples. Skipping WW.") # Can be noisy
                        continue 
                else: 
                             ...
                             if state_just_changed_to_sending:
                                 response_create_payload = {
                                     ...
                                     "response": {
                                         ...
                                         "voice": "ash", # As per your existing logic
...
if __name__ == "__main__":
    ...
    # Check for FASTAPI_DISPLAY_API_URL, warn if not set (unless in a CI test environment)
    if not APP_CONFIG.get("FASTAPI_DISPLAY_API_URL") and not os.environ.get("CI_TEST_MODE"):
        log("WARNING: FASTAPI_DISPLAY_API_URL not found in .env. Display tool will not function.")
        # If display is critical, you might choose to exit:
        # log("CRITICAL: FASTAPI_DISPLAY_API_URL not found. Exiting."); exit(1)
    ...
    if not SCIPY_AVAILABLE and wake_word_active: 
        log("WARNING: Scipy unavailable, WW was active but now disabled. App in direct OpenAI mode.")
        wake_word_active = False 
        set_app_state_main(STATE_SENDING_TO_OPENAI)
    ...
    try:
        while ws_client_thread.is_alive(): 
            if audio_pipeline_thread and not audio_pipeline_thread.is_alive() and openai_client_instance.connected:
                log("WARNING: Audio pipeline thread exited while OpenAI client connected. This might indicate an issue with mic input.")
                break 
            time.sleep(0.5) 
        ...
        # Attempt to close connection via a dedicated method if available in the client
        if hasattr(openai_client_instance, 'close_connection') and callable(openai_client_instance.close_connection):
            log("Attempting to close OpenAI WebSocket connection via client's close_connection method...")
            openai_client_instance.close_connection()
        elif hasattr(openai_client_instance, 'ws_app') and openai_client_instance.ws_app and openai_client_instance.connected:
            log("Attempting to close OpenAI WebSocket connection directly via ws_app.close()...")
            try:
                if hasattr(openai_client_instance.ws_app, 'close'):
                    openai_client_instance.ws_app.close()
            except Exception as e_ws_close:
                log(f"Exception during WebSocket ws_app.close(): {e_ws_close}")
        
        if ws_client_thread and ws_client_thread.is_alive():
            log("Waiting for OpenAI client thread to join...")
            ws_client_thread.join(timeout=3)
        ...
        if player_instance and player_instance.stream:
            log("Flushing and closing PCMPlayer stream.")
            player_instance.flush() # Ensure any remaining audio is played
