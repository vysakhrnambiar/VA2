...
APP_CONFIG = {
    ...
    "END_CONV_AUDIO_FINISH_DELAY_S": os.getenv("END_CONV_AUDIO_FINISH_DELAY_S", "2.0")
}
...
try:
    ...
    if SCIPY_AVAILABLE:
        ...
        log("Scipy unavailable for WakeWordDetector resampling. Ensure input matches model rate or WW might be INACTIVE.")
        wake_word_active = False
...
if not wake_word_active and wake_word_detector_instance is None:
    ...
    log("Using DUMMY wake word detector as fallback.")
     ...
     log("WakeWordDetector instance exists but is effectively inactive (e.g. dummy model or resampling issue).")
...
def continuous_audio_pipeline(openai_client_ref):
    ...
    if not OPENAI_API_KEY: log("CRITICAL: OpenAI API key not found. Stopping audio pipeline."); return
    ...
    log("Noise reduction for mic input: REMOVED FROM SYSTEM.")
    ...
    try:
        ...
        log("AUDIO_CAPTURE: Recording effectively RAW microphone input (no noise reduction) to mic_capture_processed.wav")
    ...
    try:
        while True:
            if not openai_client_ref.connected:
                ...
                if not (hasattr(openai_client_ref, 'ws_app') and openai_client_ref.ws_app and \
                   getattr(openai_client_ref.ws_app, 'keep_running', False)):
                    log("OpenAI WebSocket seems definitively stopped (from audio pipeline). Exiting audio loop."); break
                continue
            ...
            try:
                if mic_stream.is_active():
                    ...
                    if len(raw_audio_bytes_24k) != expected_len:
                        log(f"WARN: Mic read {len(raw_audio_bytes_24k)} bytes, expected {expected_len}. Possible overflow or timing issue. Discarding.")
                        raw_audio_bytes_24k = b''
                else: log("WARN: Mic stream not active. Sleeping."); time.sleep(CHUNK_MS / 1000.0); continue
            ...
            # Corrected WAV writing blocks
            if wf_raw:
                try:
                    wf_raw.writeframes(raw_audio_bytes_24k)
                except Exception as e_wav_raw:
                    log(f"ERROR writing raw WAV: {e_wav_raw}")
            if wf_processed:
                try:
                    wf_processed.writeframes(audio_bytes_24k_for_downstream)
                except Exception as e_wav_proc:
                    log(f"ERROR writing processed WAV: {e_wav_proc}")
            ...
            if run_local_vad_check and audio_bytes_24k_for_downstream:
                ...
                if audio_for_vad_16khz:
                    ...
                    if speech_detected_this_chunk:
                        ...
                        print(f"LOCAL_VAD: Speech detected - frame count: {local_vad_speech_frames_count}/{MIN_SPEECH_FRAMES_FOR_LOCAL_INTERRUPT}")
                        ...
                        if local_vad_speech_frames_count > 0:
                            ...
                            print(f"LOCAL_VAD: Silence after speech - frame count: {local_vad_silence_frames_after_speech}/{MIN_SILENCE_FRAMES_TO_RESET_LOCAL_VAD_STATE}")
                            if local_vad_silence_frames_after_speech >= MIN_SILENCE_FRAMES_TO_RESET_LOCAL_VAD_STATE:
                                local_vad_speech_frames_count = 0; local_vad_silence_frames_after_speech = 0
                                print("LOCAL_VAD: Reset speech detection state due to silence")
            else:
                local_vad_speech_frames_count = 0; local_vad_silence_frames_after_speech = 0
            ...
            if current_pipeline_app_state_iter == STATE_LISTENING_FOR_WAKEWORD and wake_word_active and audio_bytes_24k_for_downstream:
                if SCIPY_AVAILABLE and wake_word_detector_instance:
                    ...
                    if num_samples_16k_ww > 0:
                        ...
                        except Exception as e_ww_proc: log(f"ERROR during wake word audio processing/resampling: {e_ww_proc}")
                elif not SCIPY_AVAILABLE and wake_word_active:
                    if INPUT_RATE == WAKE_WORD_PROCESS_RATE:
                         if wake_word_detector_instance.process_audio(audio_bytes_24k_for_downstream):
                            log_section(f"WAKE WORD DETECTED (NO RESAMPLE): '{wake_word_detector_instance.wake_word_model_name.upper()}'!")
                            set_app_state_main(STATE_SENDING_TO_OPENAI); print("\n*** Wake word detected! Sending audio to OpenAI... ***\n")
                    ...
                    try:
                        if hasattr(openai_client_ref.ws_app, 'send'):
                             ...
                             if state_just_changed_to_sending:
                                 ...
                                 log("Sent initial 'response.create' after state change to SENDING_TO_OPENAI");
                                 state_just_changed_to_sending = False
        ...
        if wf_raw:
            ...
            except Exception as e_w_close: log(f"Error closing raw wav: {e_w_close}")
        if wf_processed:
            try: wf_processed.close(); log("AUDIO_CAPTURE: Closed mic_capture_processed.wav")
            except Exception as e_w_close: log(f"Error closing processed wav: {e_w_close}")
...
if __name__ == "__main__":
    ...
    log("Noise Reduction: REMOVED FROM SYSTEM.")
    ...
    else: log("WW INACTIVE (or model/resampling issue).")
    ...
    try:
        while ws_client_thread.is_alive():
            if audio_pipeline_thread and not audio_pipeline_thread.is_alive() and \
               openai_client_instance and (hasattr(openai_client_instance, 'connected') and openai_client_instance.connected):
                log("WARNING: Audio pipeline thread exited while OpenAI client still connected. Might indicate an issue in audio loop."); break
            time.sleep(0.5)
        ...
        if audio_pipeline_thread and audio_pipeline_thread.is_alive(): log("WARN: Audio pipeline thread did not join cleanly.")
        ...
        if ws_client_thread and ws_client_thread.is_alive(): log("WARN: OpenAI client thread did not join cleanly.")
