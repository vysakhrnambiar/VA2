...
def execute_send_email(subject: str, body_text: str, html_body_content: str, config: dict, is_ticket_format: bool = False) -> tuple[bool, str]:
    ...
    try:
        ...
        if 200 <= response.status_code < 300:
            resend_id = "N/A"
            try: # Try to get Resend ID from JSON response
                resend_id = response.json().get('id', 'N/A')
            except json.JSONDecodeError: # If response is not JSON, just ignore
        ...
        else: # Handle error responses (status code not 2xx)
            error_detail = response.text # Default error detail is the raw text
            try: 
                # Attempt to parse a more specific error message if the response is JSON
                error_json = response.json()
                if "message" in error_json: # Common Resend error structure
                    error_detail = error_json["message"]
                elif "error" in error_json and isinstance(error_json["error"], dict) and "message" in error_json["error"]: # Nested error structure
                    error_detail = error_json["error"]["message"]
                elif isinstance(error_json, dict) and "name" in error_json and "message" in error_json: # Another Resend error {name:..., message:...}
                    error_detail = f"{error_json['name']}: {error_json['message']}"
            except json.JSONDecodeError:
                # If parsing as JSON fails, error_detail remains the raw response.text
                _tool_log(f"WARN: Could not parse error response from Resend as JSON. Raw text: {response.text[:200]}")
                # No 'pass' needed here, error_detail is already set to response.text
            
            error_msg = f"Failed to send email. Status: {response.status_code}. Detail: {error_detail}"
            _tool_log(f"ERROR: {error_msg}")
            # Truncate long error details for the message returned to LLM
...
def handle_raise_ticket_for_missing_knowledge(user_query: str, additional_context: str = "", config: dict = None) -> str:
    ...
    html_content = f"<h2>Missing Knowledge Ticket (AI Generated)</h2><p><strong>Query:</strong><br>{user_query.replace(r'\\n', '<br>').replace('\n', '<br>')}</p><p><strong>Context:</strong><br>{(additional_context or 'N/A').replace(r'\\n', '<br>').replace('\n', '<br>')}</p>"
...
def handle_display_on_interface(display_type: str, data: dict, config: dict, title: str = None) -> str:
    ...
    if display_type == "markdown":
        ...
        if not isinstance(data.get("labels"), list) or not isinstance(data.get("datasets"), list) or not data.get("datasets"): # Ensure datasets is not empty if it's a list
            return "Error: Invalid data for graph: 'labels' or 'datasets' missing/invalid or 'datasets' is empty."
        for i, dataset in enumerate(data["datasets"]): # Add get() for safety
    ...
    except json.JSONDecodeError: # If FastAPI response is not JSON
